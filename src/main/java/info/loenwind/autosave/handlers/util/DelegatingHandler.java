...
public class DelegatingHandler<T, R> implements IHandler<T> {
     ...
     */
    public DelegatingHandler(Class<T> type, IHandler<R> delegate,
                             Function<@NonnullType T, R> storeConverter,
                             Function<@NonnullType R, T> readConverter) {
        this((Type) type, delegate, storeConverter, readConverter);
    }

    public DelegatingHandler(Type type, IHandler<R> delegate,
                             Function<@NonnullType T, R> storeConverter,
                             Function<@NonnullType R, T> readConverter) {
        this.type = type;
    ...
    @Override
    public boolean store(Registry registry, Set<NBTAction> phase,
                         NBTTagCompound nbt, Type type,
                         String name, T object) throws IllegalArgumentException,
    ...
    @Override
    @Nullable
    public T read(Registry registry, Set<NBTAction> phase,
                  NBTTagCompound nbt, Type type,
                  String name, @Nullable T object) throws IllegalArgumentException,
                                                   IllegalAccessException,
                                                   InstantiationException,
                                                   NoHandlerFoundException {
        R intermediate = delegate.read(registry, phase, nbt, type, name,
                object == null ? null : storeConverter.apply(object));
