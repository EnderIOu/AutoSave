...
public abstract class HandleGenericType<T> implements IHandler<T> {
    ...
    protected HandleGenericType(Class<? extends T> clazz, Registry registry,
                                Type... parameterTypes) throws NoHandlerFoundException {
        this(clazz);
        if (parameterTypes.length != getRequiredParameters()) {
            throw new IllegalArgumentException("Mismatch of parameter count. Required: " + getRequiredParameters() +
                    "  Found: " + parameterTypes.length);
        }
    ...
    protected abstract @Nullable IHandler<? extends T> create(Registry registry,
    ...
    public @Nullable IHandler<? extends T> getHandler(Registry registry, Type type) {
        ...
        if (type instanceof ParameterizedType) {
            try {
                return create(registry, NullHelper.notnullJ(((ParameterizedType) type).getActualTypeArguments(),
    ...
    @SuppressWarnings("unchecked")
    protected final void storeRecursive(int param, Registry registry, Set<NBTAction> phase,
                                        NBTTagCompound nbt, String name, Object object) throws IllegalArgumentException,
    ...
    protected final @Nullable <
            V> V readRecursive(int param, Registry registry, Set<NBTAction> phase,
                               NBTTagCompound nbt, String name, @Nullable V object) throws IllegalArgumentException,
                                                                                    IllegalAccessException,
                                                                                    InstantiationException,
                                                                                    NoHandlerFoundException {
        for (IHandler handler : subHandlers[param]) {
